...
public class AdminController {
    ...
    @FXML
    private TextField nuevoUsuarioField;
    @FXML
    private TextField nuevoEmailField;
    @FXML
    private PasswordField nuevoContrasenaField;
    @FXML
    private ComboBox<String> rolCombo;
    @FXML
    private Label estadoAdminLabel;
    @FXML
    private Label currentUserLabel;

    // Paneles
    @FXML
    private javafx.scene.layout.VBox usuariosPane;
    @FXML
    private javafx.scene.layout.VBox promocionesPane;
    @FXML
    private javafx.scene.layout.VBox tarifasPane;
    @FXML
    private javafx.scene.layout.VBox recursosPane;
    @FXML
    private javafx.scene.layout.VBox reportesPane;

    // Promociones
    @FXML
    private TableView<Promocion> promosTable;
    @FXML
    private TableColumn<Promocion, Integer> colPromoId;
    @FXML
    private TableColumn<Promocion, String> colPromoNombre;
    @FXML
    private TableColumn<Promocion, String> colPromoTipo;
    @FXML
    private TableColumn<Promocion, Double> colPromoValor;
    @FXML
    private TableColumn<Promocion, String> colPromoEstado;
    @FXML
    private TextField promoNombreField;
    @FXML
    private ComboBox<String> promoTipoCombo;
    @FXML
    private TextField promoValorField;
    @FXML
    private ComboBox<String> promoEstadoCombo;
    ...
    @FXML
    private ListView<String> listaMantenimiento;

    @FXML
    private javafx.scene.control.DatePicker desdePicker;
    @FXML
    private javafx.scene.control.DatePicker hastaPicker;
    @FXML
    private Label lblMasAlquilado;
    @FXML
    private javafx.scene.chart.BarChart<String, Number> barMontos;
    @FXML
    private javafx.scene.chart.LineChart<String, Number> lineTendencias;
    @FXML
    private javafx.scene.control.ListView<String> listaClientesFrecuentes;
    @FXML
    private javafx.scene.control.ProgressIndicator exportProgress;
    @FXML
    private javafx.scene.control.Label exportStatus;

    // Dashboard nuevos elementos
    @FXML
    private javafx.scene.control.Label metricStudents;
    @FXML
    private javafx.scene.control.Label metricTeachers;
    @FXML
    private javafx.scene.control.Label metricParents;
    @FXML
    private javafx.scene.control.Label metricEarnings;
    @FXML
    private javafx.scene.chart.BarChart<String, Number> examResultsBar;
    @FXML
    private javafx.scene.chart.PieChart genderPie;
    @FXML
    private javafx.scene.control.ListView<String> dashboardNotifications;
    @FXML
    private javafx.scene.layout.VBox dashboardPane;
    @FXML
    private javafx.scene.layout.StackPane contenidoStack;
    ...
    @FXML
    private TableView<com.playa.alquiler.model.Recurso> recursosTable;
    @FXML
    private TableColumn<com.playa.alquiler.model.Recurso, Integer> colRecId;
    @FXML
    private TableColumn<com.playa.alquiler.model.Recurso, String> colRecNombre;
    @FXML
    private TableColumn<com.playa.alquiler.model.Recurso, String> colRecTipo;
    @FXML
    private TableColumn<com.playa.alquiler.model.Recurso, String> colRecEstado;
    @FXML
    private TextField recursoNombreField;
    @FXML
    private TextField recursoDescripcionField;
    @FXML
    private TextField recursoTipoField;
    @FXML
    private ComboBox<String> recursoEstadoCombo;

    @FXML
    public void initialize() {
        // Mostrar usuario actual en la barra superior
        if (CurrentSession.getUsuario() != null) {
            currentUserLabel.setText("Usuario: " + CurrentSession.getUsuario().getNombreUsuario() + " ("
                    + CurrentSession.getNombreRol() + ")");
        }
        ...
        if (recursosTable != null) {
            ...
            recursoEstadoCombo
                    .setItems(FXCollections.observableArrayList("Disponible", "En Mantenimiento", "Alquilado"));
            recargarRecursos();
        ...
        try {
            ...
            if (metricStudents != null)
                metricStudents.setText(String.valueOf(enCurso));
        } catch (SQLException ignored) {
        }
        try {
            int finalizados = new AlquilerDAO().contarPorEstado("Finalizado");
            if (metricTeachers != null)
                metricTeachers.setText(String.valueOf(finalizados));
        } catch (SQLException ignored) {
        }
        try {
            int disponibles = new com.playa.alquiler.dao.RecursoDAO().buscarRecursosDisponibles().size();
            if (metricParents != null)
                metricParents.setText(String.valueOf(disponibles));
        } catch (SQLException ignored) {
        }
        try {
            ...
            if (metricEarnings != null)
                metricEarnings.setText("S/" + total);
        ...
        try {
            if (examResultsBar != null) {
                ...
                try (java.sql.Connection conn = com.playa.alquiler.db.ConexionDB.getConnection();
                        java.sql.PreparedStatement ps = conn.prepareStatement(
                                "SELECT FORMAT(a.fecha,'yyyy-MM') AS mes, COUNT(*) AS c FROM Alquileres a WHERE a.fecha BETWEEN ? AND ? GROUP BY FORMAT(a.fecha,'yyyy-MM') ORDER BY mes")) {
                    ps.setDate(1, java.sql.Date.valueOf(d));
                    ps.setDate(2, java.sql.Date.valueOf(h));
                    try (java.sql.ResultSet rs = ps.executeQuery()) {
                        while (rs.next())
                ...
                for (var e : datos.entrySet())
                    s.getData().add(new javafx.scene.chart.XYChart.Data<>(e.getKey(), e.getValue()));
    ...
    private void switchTo(javafx.scene.Node target) {
        if (contenidoStack == null || target == null)
            return;
        for (javafx.scene.Node child : contenidoStack.getChildren()) {
    ...
    public void mostrarGestionTarifas(ActionEvent e) {
        // Redirigir a recursos ya que tarifas están integradas
        ...
        try {
            ...
            if (desdePicker != null && hastaPicker != null) {
                if (desdePicker.getValue() == null)
                    desdePicker.setValue(java.time.LocalDate.now().minusDays(30));
                if (hastaPicker.getValue() == null)
                    hastaPicker.setValue(java.time.LocalDate.now());
            }
    ...
    public void onActualizarRecurso(ActionEvent e) {
        ...
        if (sel == null) {
            estadoAdminLabel.setText("Seleccione un recurso");
            return;
        }
        try {
    ...
    public void onAgregarTarifaRecurso(ActionEvent e) {
        // Deprecated
        estadoAdminLabel.setText("Funcionalidad movida a Recurso");
    }
        ...
        try {
            ...
            try (java.sql.Connection conn = com.playa.alquiler.db.ConexionDB.getConnection()) {
                java.sql.PreparedStatement psT = conn.prepareStatement(
                        "SELECT COUNT(*) FROM detalle_alquiler d JOIN Alquileres a ON a.alquiler_id=d.alquiler_id WHERE a.fecha BETWEEN ? AND ?");
                psT.setDate(1, java.sql.Date.valueOf(d));
                psT.setDate(2, java.sql.Date.valueOf(h));
                long totalGeneral = 0;
                try (java.sql.ResultSet rs = psT.executeQuery()) {
                    if (rs.next())
                        totalGeneral = rs.getLong(1);
                }
                java.sql.PreparedStatement ps = conn.prepareStatement(
                        "SELECT TOP 1 d.recurso_id, COUNT(*) AS total FROM detalle_alquiler d JOIN Alquileres a ON a.alquiler_id=d.alquiler_id WHERE a.fecha BETWEEN ? AND ? GROUP BY d.recurso_id ORDER BY total DESC, d.recurso_id");
                ps.setDate(1, java.sql.Date.valueOf(d));
                ps.setDate(2, java.sql.Date.valueOf(h));
                try (java.sql.ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        ...
                        lblMasAlquilado.setText("Recurso: " + nombre + " • Alquileres: " + cant + " • Utilización: "
    ...
    private void cargarTendencias(java.time.LocalDate d, java.time.LocalDate h) throws SQLException {
        ...
        if (lineTendencias != null) {
            ...
            for (var entry : datos.entrySet())
                s.getData().add(new javafx.scene.chart.XYChart.Data<>(entry.getKey(), entry.getValue()));
    ...
    private void cargarClientesFrecuentes(java.time.LocalDate d, java.time.LocalDate h) throws SQLException {
        ...
        try (java.sql.Connection conn = com.playa.alquiler.db.ConexionDB.getConnection();
                java.sql.PreparedStatement ps = conn.prepareStatement(
                        "SELECT TOP 5 t.nombres + ' ' + t.apellidos AS nombre, COUNT(*) AS c FROM Alquileres a JOIN Turista t ON t.id_turista=a.id_turista WHERE a.fecha BETWEEN ? AND ? GROUP BY t.nombres, t.apellidos ORDER BY c DESC")) {
            ps.setDate(1, java.sql.Date.valueOf(d));
            ps.setDate(2, java.sql.Date.valueOf(h));
            try (java.sql.ResultSet rs = ps.executeQuery()) {
                while (rs.next())
                    items.add(rs.getString(1) + " (" + rs.getInt(2) + ")");
            }
        }
    ...
    public void onMontosGenerados(ActionEvent e) {
        try {
            ...
            java.math.BigDecimal total = java.math.BigDecimal.ZERO;
            // Use stored total_a_pagar from detalle_alquiler instead of recalculating with
            // current tariff
            try (java.sql.Connection conn = com.playa.alquiler.db.ConexionDB.getConnection();
                    java.sql.PreparedStatement ps = conn.prepareStatement(
                            "SELECT r.tipo_de_recurso, SUM(d.total_a_pagar) FROM detalle_alquiler d JOIN Alquileres a ON a.alquiler_id=d.alquiler_id JOIN Recursos r ON r.id_recurso=d.recurso_id WHERE a.fecha BETWEEN ? AND ? GROUP BY r.tipo_de_recurso")) {
                ps.setDate(1, java.sql.Date.valueOf(d));
                ps.setDate(2, java.sql.Date.valueOf(h));
                try (java.sql.ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
                        String tipo = rs.getString(1);
                        java.math.BigDecimal monto = rs.getBigDecimal(2);
                        if (monto == null)
                            monto = java.math.BigDecimal.ZERO;
                        porTipo.put(tipo == null ? "Sin Tipo" : tipo, monto);
            ...
            if (barMontos != null) {
                ...
                java.util.List<java.util.Map.Entry<String, java.math.BigDecimal>> entries = new java.util.ArrayList<>(
                        porTipo.entrySet());
                entries.sort((a, b) -> b.getValue().compareTo(a.getValue()));
                javafx.scene.chart.XYChart.Series<String, Number> serie = new javafx.scene.chart.XYChart.Series<>();
                for (var entry : entries)
                    serie.getData().add(new javafx.scene.chart.XYChart.Data<>(entry.getKey(), entry.getValue()));
                var xAxis = (javafx.scene.chart.CategoryAxis) barMontos.getXAxis();
                xAxis.setCategories(javafx.collections.FXCollections.observableArrayList(entries.stream()
                        .map(java.util.Map.Entry::getKey).collect(java.util.stream.Collectors.toList())));
    ...
    public void onCrearUsuario(ActionEvent e) {
        ...
        if (nombre == null || nombre.isBlank() || email == null || email.isBlank() || contrasena == null
                || contrasena.isBlank() || rolNombre == null) {
    ...
    public void onCrearPromocion(ActionEvent e) {
        ...
        if (nombre == null || nombre.isBlank() || tipo == null || valorTxt == null || valorTxt.isBlank()
                || estado == null) {
    ...
    public void onEliminarPromocion(ActionEvent e) {
        ...
        if (sel == null) {
            estadoAdminLabel.setText("Seleccione una promoción.");
            return;
        }
        try {
    ...
    public void onLogout(ActionEvent e) {
        ...
        try {
            javafx.scene.Parent root = javafx.fxml.FXMLLoader
                    .load(getClass().getResource("/com/playa/alquiler/view/LoginView.fxml"));
    ...
    public void onExportarPDF(ActionEvent e) {
        try {
            ...
            if (exportStatus != null) {
                exportStatus.setText("Exportando PDF...");
            }
            ...
            try {
                ...
                String periodo = "Período: " + (desdePicker != null ? String.valueOf(desdePicker.getValue()) : "-")
                        + " a " + (hastaPicker != null ? String.valueOf(hastaPicker.getValue()) : "-");
                cs.showText(periodo);
                cs.endText();
                y -= 24;
                cs.beginText();
                ...
                doc.save(file);
                if (exportStatus != null) {
                    exportStatus.setText("PDF exportado: " + file.getAbsolutePath());
                }
            } finally {
                doc.close();
                if (exportProgress != null) {
            ...
            if (exportStatus != null) {
                exportStatus.setText("Error exportando PDF: " + ex.getMessage());
            }
    ...
    public void onExportarExcel(ActionEvent e) {
        try {
            ...
            if (exportStatus != null) {
                exportStatus.setText("Exportando Excel...");
            }
            javafx.stage.FileChooser fc = new javafx.stage.FileChooser();
            ...
            try {
                ...
                r2.createCell(0)
                        .setCellValue("Período: " + (desdePicker != null ? String.valueOf(desdePicker.getValue()) : "-")
                                + " a " + (hastaPicker != null ? String.valueOf(hastaPicker.getValue()) : "-"));
                ...
                shMontos.autoSizeColumn(0);
                shMontos.autoSizeColumn(1);
                ...
                if (exportStatus != null) {
                    exportStatus.setText("Excel exportado: " + file.getAbsolutePath());
                }
            } finally {
                wb.close();
                if (exportProgress != null) {
            ...
            if (exportStatus != null) {
                exportStatus.setText("Error exportando Excel: " + ex.getMessage());
            }
