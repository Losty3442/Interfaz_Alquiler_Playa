...
public class PagoService {
    ...
    public ResumenVentas ventasDiariasPorVendedor(int usuarioId, LocalDate fecha) throws SQLException {
        String sql = "SELECT COALESCE(SUM(d.total_a_pagar),0) AS total, COUNT(DISTINCT a.alquiler_id) AS cantidad " +
                "FROM Alquileres a JOIN detalle_alquiler d ON a.alquiler_id = d.alquiler_id " +
        ...
        try (Connection conn = ConexionDB.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    LOGGER.info("Ventas diarias usuario=" + usuarioId + " fecha=" + fecha + " total=" + total + " cantidad=" + cantidad);
                    return new ResumenVentas(total, cantidad);
                }
            }
            return new ResumenVentas(BigDecimal.ZERO, 0);
        }
    }
}
