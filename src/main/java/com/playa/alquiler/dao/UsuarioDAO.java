...
public class UsuarioDAO {
    ...
    /**
     * Búsqueda directa por nombre de usuario - optimizado para login rápido
     */
    public Usuario obtenerPorNombreUsuario(String nombreUsuario) throws SQLException {
        String sql = "SELECT usuario_id, nombre_usuario, email, contraseña, rol_id FROM Usuario WHERE nombre_usuario=?";
        try (Connection conn = ConexionDB.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, nombreUsuario);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
